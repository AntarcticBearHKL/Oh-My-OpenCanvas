...
+ [新增] 画布新增「框架」节点：可把节点拖入框架内成为一组，拖动框架时子节点跟随移动，删除框架只释放子节点、不删除节点内容。
...
+ [新增] 画布 MCP 新增节点工具：设置锁定/隐藏（canvas_set_node_flags）、批量重命名（canvas_bulk_rename，多选按顺序编号）、对齐与等距分布（canvas_align_nodes）、组合（canvas_group_nodes）、取消组合（canvas_ungroup_nodes）与复制节点（canvas_duplicate_node）；节点 metadata 支持显式传 null 清除字段（如解除分组的 groupId）。
