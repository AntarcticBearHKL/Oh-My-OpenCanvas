...
export function normalizeConnection(firstNodeId: string, secondNodeId: string, nodes: CanvasNodeData[], firstHandleType: "source" | "target") {
    ...
    const isGenerationSink = (type: CanvasNodeTypeId) => type === CanvasNodeType.Config || type === CanvasNodeType.ImageGeneration || type === CanvasNodeType.AudioGeneration || type === CanvasNodeType.SpeechGeneration || type === CanvasNodeType.MusicGeneration;
