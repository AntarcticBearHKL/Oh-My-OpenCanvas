...
export function CanvasNodePromptPanel({ node, nodes, isRunning, onPromptChange, onConfigChange, onGenerate, onStop, mentionReferences = [], connectedNodes = [], onDisconnectReference, onStartReferenceSelection, onImageSettingsOpenChange, modeOverride }: CanvasNodePromptPanelProps) {
    ...
    const requiredPromptType = node.type === CanvasNodeType.MusicGeneration ? CanvasNodeType.MusicPrompt : CanvasNodeType.SpeechPrompt;
    ...
    return (
        ...
        >
            {acceptsPromptConnection ? null : <CanvasNodeReferenceBar nodeId={node.id} connectedNodes={connectedNodes} onDisconnect={onDisconnectReference} onStartSelection={onStartReferenceSelection} />}
            ...
            <div className="mt-2 flex min-w-0 items-center justify-between gap-2" onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}>
                <div className="flex min-w-0 items-center gap-2">
                    ...
                    {mode === "image" ? (
                        ...
                        <>
                            <ModelPicker config={config} value={config.model} onChange={(model) => onConfigChange(node.id, { model })} capability="video" onMissingConfig={() => openConfigDialog()} className="max-w-[190px]" />
