    ...
    return (
        <>
            <button ref={buttonRef} type="button" className="group relative flex h-12 items-center whitespace-nowrap px-1.5" style={{ color: theme.node.text }} aria-label={t("canvas.nodeToolbar.textStyle")} onClick={() => setOpen((current) => !current)}>
                <span className="flex h-9 items-center gap-2 rounded-lg px-2.5 transition group-hover:bg-black/5 dark:group-hover:bg-card/10" style={open ? { background: theme.toolbar.activeBg, color: theme.toolbar.activeText } : undefined}>
