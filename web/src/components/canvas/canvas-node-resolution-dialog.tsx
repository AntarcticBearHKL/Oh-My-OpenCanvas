...
export function CanvasNodeResolutionDialog({ dataUrl, open, onClose, onConfirm }: { dataUrl: string; open: boolean; onClose: () => void; onConfirm: (payload: CanvasImageResolutionPayload) => void }) {
                ...
                {algorithms.map((algorithm) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="mt-0.5 text-xs opacity-55">{t(`canvas.editors.${algorithm}Description`)}</div>
                        </button>
                    );
                })}
            </div>
        ...
        <div className="space-y-6 py-2">
            <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                    <span className="shrink-0 text-sm opacity-60">{t("canvas.editors.resolutionCustom")}</span>
                ...
                {image && !canDownscale ? <div className="text-xs font-medium text-[#ef4444]">{t("canvas.editors.resolutionDownscaleRequired")}</div> : null}
            </div>
            {renderAlgorithm()}
            {renderOutputSize()}
            <div className="flex justify-end">
    ...
    const aiTab = (
        <div className="space-y-6 py-2">
            <div className="text-sm leading-6 opacity-70">{t("canvas.editors.resolutionAiHint")}</div>
            <div className="space-y-2">
                <div className="font-medium opacity-75">{t("canvas.editors.resolutionAiFactor")}</div>
                <Segmented block value={aiFactor} options={["2x", "4x"].map((value) => ({ label: value, value }))} onChange={(value) => setAiFactor(String(value))} />
            </div>
            <div className="space-y-2">
                <div className="font-medium opacity-75">{t("canvas.editors.resolutionAiPrompt")}</div>
                <Input.TextArea rows={5} value={aiPrompt} onChange={(event) => setAiPrompt(event.target.value)} />
            </div>
            <div className="rounded-xl border px-4 py-3 text-sm">
                <div className="flex items-center justify-between gap-3">
                    <span className="shrink-0 opacity-60">{t("canvas.editors.resolutionAiModel")}</span>
