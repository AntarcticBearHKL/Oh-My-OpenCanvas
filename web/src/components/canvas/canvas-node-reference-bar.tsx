...
export function CanvasNodeReferenceBar({ nodeId, connectedNodes, onDisconnect, onStartSelection }: { nodeId: string; connectedNodes: CanvasNodeData[]; onDisconnect?: (fromNodeId: string, toNodeId: string) => void; onStartSelection?: (nodeId: string) => void }) {
    ...
    const references = connectedNodes.map((node) => ({ node, sourceNodeId: node.id }));
    return (
        <div className="mb-2">
            ...
            <div className="thin-scrollbar flex min-h-12 gap-2 overflow-x-auto pb-1">
                ...
                <button type="button" className="grid size-12 shrink-0 place-items-center rounded-xl border bg-transparent transition hover:opacity-70" style={{ borderColor: theme.toolbar.border, color: theme.node.muted }} title={t("canvas.references.select")} onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()} onClick={() => onStartSelection?.(nodeId)}>
