...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    return (
        ...
        >
            ...
            >
                {isPlacedOnBoard && !referenceSelectionState ? (
                    <div className="pointer-events-none absolute left-2 top-2 z-40 flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-medium" style={{ background: theme.toolbar.panel, color: theme.node.muted }}>
                        <LayoutDashboard className="size-3" />
                        {t("canvas.smartCanvas.placedOnBoard")}
                    </div>
                ) : null}
                <div
                ...
                {referenceSelectionState && (referenceSelectionState !== "available" || hovered) ? (
                    <div className="pointer-events-none absolute inset-0 z-[60] grid place-items-center rounded-[inherit]" style={{ background: `color-mix(in srgb, ${theme.canvas.background} ${referenceSelectionState === "target" ? 78 : referenceSelectionState === "disabled" ? 60 : 34}%, transparent)`, boxShadow: referenceSelectionState === "available" ? `inset 0 0 0 2px ${selectionBlue}` : undefined }}>
                        {referenceSelectionState !== "disabled" ? <span className="rounded-lg px-3 py-2 text-sm font-medium" style={{ background: theme.toolbar.panel, color: theme.node.text }}>{t(referenceSelectionState === "target" ? "canvas.references.selecting" : "canvas.references.choose")}</span> : null}
                    </div>
                ) : null}
            ...
            {!referenceSelectionState && data.type !== CanvasNodeType.Image ? <ConnectionHandleDot side="left" visible={hovered || isSelected || isConnecting} onMouseDown={(event) => onConnectStart(event, data.id, "target")} /> : null}
...
function NodeContent(props: NodeContentRendererProps) {
    if ((props.node.type === CanvasNodeType.Config || props.node.type === CanvasNodeType.ImageGeneration || props.node.type === CanvasNodeType.AudioGeneration || props.node.type === CanvasNodeType.Prompt || props.node.type === CanvasNodeType.Assets) && props.renderNodeContent) return props.renderNodeContent(props.node);
        ...
        >
            {text.content ? (
                <>
                    ...
                    <button type="button" className="absolute right-2.5 top-2.5 flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-xs font-medium transition hover:bg-black/5 dark:hover:bg-card/10" style={{ color: theme.node.text }} onClick={(event) => (event.stopPropagation(), onSetPrimary())}>
...
function ImageInfoBar({ node, onInfo }: { node: CanvasNodeData; onInfo?: (node: CanvasNodeData) => void }) {
    ...
    return (
        <div className="pointer-events-none absolute left-3 top-[-28px] z-40 flex max-w-[calc(100%-24px)] items-center gap-1.5">
            ...
            <button
                type="button"
                className="pointer-events-auto grid size-5 shrink-0 place-items-center rounded-full transition hover:bg-black/5 dark:hover:bg-card/10"
